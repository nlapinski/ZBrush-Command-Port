...
class ZBrushGUI(object):
    ...
    def serv_start(self):
        ...
        status_line = 'ZBrushServer Status: %s:%s' % (self.serv.host, self.serv.port)
 
        self.zbrush_status_ui.config(text=status_line,background='green')

    def serv_stop(self):
        self.serv.stop()
        self.zbrush_status_ui.config(text = 'ZBrushServer Status: down' ,background='red')
    ...
    def test_client(self):
        ...
        if self.client.test_client():
            ...
            self.maya_status_ui.config(text = 'MayaClient Status: connected' ,background='green')
        else:
            self.maya_status_ui.config(text = 'MayaClient Status: conn refused' ,background='red')
           

    def build(self):
        self.win = Tkinter.Tk()
        self.win.title('GoZ GUI')
        Tkinter.Label(self.win,text='GoZ - Basic Setup:').pack(pady=5,padx=25)

        Tkinter.Label(self.win,text='Set hosts/ports in default.cfg').pack(pady=0,padx=25)
        Tkinter.Label(self.win,text='If not set starts in local mode').pack(pady=0,padx=25)
        Tkinter.Label(self.win,text='ZBrush should be launched first').pack(pady=0,padx=25)
        Tkinter.Label(self.win,text='Everything should be automatic').pack(pady=0,padx=25)
        Tkinter.Label(self.win,text='Changes are added to default.cfg').pack(pady=0,padx=25)
        ...
        Tkinter.Button(zb_cfg, text='Start ZBrushServer',command=self.serv_start).pack()
        Tkinter.Button(zb_cfg, text='Stop ZBrushServer',command=self.serv_stop).pack()

        self.zbrush_status_ui = Tkinter.Label(zb_cfg,text='ZBrushServer Status: down',background='red')
        ...
        Tkinter.Button(maya_cfg, text='Make ZBrush UI',command=self.zscript_ui).pack()
        Tkinter.Button(maya_cfg, text='Test Connection',command=self.test_client).pack()
        self.maya_status_ui = Tkinter.Label(maya_cfg,text='MayaClient Status: conn refused',background='red')
